...
kdc_addr="$(docker inspect $cid --format="{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}")"

sed "s@__HOST_NAME__@${host_name}@g; s@__SERVICE_KEYTAB__@${service_keytab}@g" "$DIR/src/login.conf" > "$DIR/target/login.conf"
sed "s@__KDC_PORT__@${kdc_port}@g; s@__ADMIN_PORT__@${admin_port}@g; s@__ADDRESS__@${kdc_addr}@g" "$DIR/src/etc.krb5.conf" > "$DIR/target/etc.krb5.conf"
